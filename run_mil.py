...
import preprocess
import coco_voc # coco_voc.__name__ = 'coco_voc'
...
def parse_args():
  ...
  # sys.argv: stores the list of command line arguments passed to a Python script. arg[0] is the script name
  if len(sys.argv) == 1:
    parser.print_help()
    sys.exit(1)
...
if __name__ == '__main__':
  ...
  if args.task == 'compute_targets':
    ...
    for i, imset in enumerate([args.train_set, args.val_set]):
      ...
      # counts: numpy array of shape (82783, 1000),
      # 1000 is the size of the vocab
      counts = preprocess.get_vocab_counts(imdb[i].image_index, \
          imdb[i].coco_caption_data, 5, vocab)

      if args.write_labels:
        label_file = os.path.join(output_dir, 'labels_' + sets[i] + '.h5') # output/v1/labels_train.h5
        ...
        with h5py.File(label_file, 'w') as f:
          for j in xrange(imdb[i].num_images):
            ...
            ind_str = '{:02d}/{:d}'.format(int(math.floor(ind)/1e4), ind) # ex: j = 0, ind_str = '00/9'
            l = f.create_dataset('/labels-{}'.format(ind_str), (1, 1, counts.shape[1], 1), dtype = 'f')
            # after statement c = c > 0, c will become a numpy array of bool(mask), elements greater than 0 are true.
            # after c.astype(np.float32), True becomes 1.0, False becomes 0.0
      ...
      if args.write_splits:
        split_file = os.path.join(output_dir, sets[i] + '.ids') # /output/v1/train.ids
  ...
  if args.task == 'output_words':
    # out_dir = out/vgg/snapshot_iter_240000.caffemodel_output
    out_dir = args.model + '_output'

    imdb = coco_voc.coco_voc(args.test_set)
    # detection_file =  out/vgg/snapshot_iter_240000.caffemodel_output/coco_valid1_detections.pkl
    detection_file = os.path.join(out_dir, imdb.name + '_detections.pkl')
